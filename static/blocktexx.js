(() => {
  ...
  function summary(s) {
    ...
    const scenario=model.weight_history?.pickups?.length?window.BlocktexxCosts.periodSummary(d,null,model.sites):null;
  ...
  function renderMetrics() {
    ...
    window.BlocktexxCosts?.renderComparison($('bx-cost-comparison'),model.states[state],v.pending,model.weight_history?.pickups?.length?window.BlocktexxCosts.periodSummary(model.states[state],null,model.sites):null);
    ...
    $('bx-gaps').textContent = `${v.pending} gaps: missing frequency/measurements, unresolved truck capacity, unassigned customers or customer frequencies that differ from the route plan. ${v.estimates} active rows use estimates. Customer frequency edits update visit demand immediately; revise affected grouped runs to update kilometres, hours and costs. Historical source kg remain unchanged; projected calendar kg follow the sample averages and your scenario overrides. All entered local movements share these hours. Interstate transfers have their own calendar and cost centre.`;
  }
